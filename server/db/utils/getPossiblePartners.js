...
const getPossiblePartnersAsync = async (requestorId, requestorType, reqSelectedTopics) => {
  ...
  const potentialPartners = await Promise.all(
    activeWannabePartners.map(async wannabe => {
      if(requestorType === 'mentee'){
        //want mentors
          wannabePartnerUserTopicInstances = await UserTopic.findAll({
            where: {
              ...
              proficiencyRating: {
                [Sequelize.Op.gte]:
                requestorUserTopicInstances[0].dataValues.proficiencyRating + 50,
              }
            }
          });
        // }
        ...
        //want peers
        const matchingReqUserTopic = requestorUserTopicInstances.filter(uTop => uTop.topicId === wannabe.selectedTopics[0]);
        ...
        if(wannabePartnerUserTopicInstances){
          return { 
            ...
            selectedTopics: {
              ...
              proficiencyRating: wannabePartnerUserTopicInstances.proficiencyRating
            },
          };
        }
        break;
      ...
      case"peer":
        if(wannabePartnerUserTopicInstances.length){
          return { 
            ...
            location: wannabe.location,
            userType: wannabe.userType,
            selectedTopics: wannabePartnerUserTopicInstances.map(uti => ({topicId: uti.topicId, proficiencyRating: uti.proficiencyRating})),
          };
        }
        break;
    }
