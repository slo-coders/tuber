...
const {UserMeetup, UserTopic, Meetup, MeetupTopic} = require('../models/index');

const findAMatch = async (reqUserId, reqUserType, location, selectedTopicsArr) => {
  return await getPossibleParnersAsync(reqUserId, reqUserType, selectedTopicsArr)
  .then(async possiblePartnersArrForRequestor => {
    ...
    if (possiblePartnersArrForRequestor.length) {
      const meetup = await Meetup.create({
        location,
        meetupType:
          reqUserType === 'mentee' || reqUserType === 'mentor'
          ? 'M:M'
          : 'P:P',
      });
      ...
      const requetorsUserTopicInstance = await UserTopic.findOne({
        where: {
          userId: reqUserId,
          topicId: reqUserType === 'mentor'
            ? possiblePartnersArrForRequestor[0].selectedTopics[0] 
            : selectedTopicsArr[0],
        },
      });
      // CREATE Requestor's UserMeetup
      const reqUserMeetupInstance = await UserMeetup.create({
        userId: reqUserId,
      ...
      const { userId: partnerId, userType: partnerType, selectedTopics } = partner;
      const partnerProfRating = partnerType === 'mentor' 
        ? selectedTopics.filter(utiObj => 
          utiObj.topicId === requetorsUserTopicInstance.topicId)[0].proficiencyRating 
        : selectedTopics[0].proficiencyRating;
  
      const partnerUserMeetupInstance = await UserMeetup.create({ 
        userId: partnerId,
