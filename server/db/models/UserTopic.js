...
UserTopic.createArr = async function (userId, arrOfUserTopics) {//`this` binds to this function's parent
  return await Promise.all(
    arrOfUserTopics.map(async uTop => {
      const [uTopReturned] = await this.findOrCreate({//`this` will will not bind to parent of arrow function
        where: {
          userId,
          topicId: uTop.topicId
        }
      });
      return await uTopReturned.update({proficiencyRating: uTop.proficiencyRating});
    }
  ));
};
